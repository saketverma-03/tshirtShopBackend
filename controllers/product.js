...
exports.getProductById = (req, res, next, id) => {
    Product.findById(id)
        .populate("category")
        .exec((error, product) => {
            iferror(error, "Not Able To get the product")
            req.product = product;
            next()
        })
}
...
exports.createProduct = (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        iferror(err, "image file is invalid");
        ...
        product.save((error, product) => {
            iferror(error, "Saving tshirt to Db failed");
            res.json({ product })
        })
    })
}
...
exports.updateStock = (req, res, next) => {
    ...
    Product.bulkWrite(myOperations, {}, (error, product) => {
        iferror(error, "Bulk  Operations failed");
        next();
    })
...
exports.removeProduct = (req, res) => {
    ...
    product.remove((error, deletedProduct) => {
        iferror(error, "failed To delete The product")
...
exports.getAllProducts = (req, res) => {
    ...
    Procduct.find()
        ...
        .sort([[sortBy, "asc"]])
        .exce((error, product) => {
            iferror(error, "Can Not load products");
            res.json(product)
        })
}
...
exports.getAllCategory = (req, res) => {
    Procduct.distinct("category", {}, (err, categorys) => {
        iferror(err, "no category found");
        res.json(categorys);
    })
}